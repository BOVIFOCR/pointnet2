...
# BERNARDO
class TreeFFHQ_3DReconstructedMICA:
    ...
    def get_all_pointclouds_paths(self, dir_path, dataset_name=None, dir_level=2, pc_ext='.ply'):
        ...
        for sub_folder_pointcloud in all_sub_folders:
            ...
            pc_subjects = [pc_path.split('/')[-(dir_level)] for pc_path in pc_paths]
            if not dataset_name is None:
                pc_subjects[0] = dataset_name + '_' + pc_subjects[0]
    ...
    def count_samples_per_subject(self, pc_paths_list=[''], pc_subjects=[''], dir_level=2, pc_ext='.ply'):
        unique_subjects_names = []
        samples_per_subject = []
        indexes_samples = []
        # for i, pc_path in enumerate(sorted(pc_paths_list)):
        for i, (pc_path, subject_name) in enumerate(zip(pc_paths_list, pc_subjects)):
    ...
    def get_all_pointclouds_paths_count(self, dir_path, dataset_name=None, dir_level=2, pc_ext='.ply'):
        print('get_all_pointclouds_paths_count() - getting all pointclouds paths ...')
        all_pc_paths, all_pc_subjects = self.get_all_pointclouds_paths(dir_path, dataset_name, dir_level, pc_ext)
        print('get_all_pointclouds_paths_count() - counting samples per subject ...')
        unique_subjects_names, samples_per_subject, indexes_samples = self.count_samples_per_subject(all_pc_paths, all_pc_subjects, dir_level, pc_ext)
        return all_pc_paths, all_pc_subjects, unique_subjects_names, samples_per_subject, indexes_samples

    def filter_paths_by_minimum_samples(self, all_pc_paths, all_pc_subjects, unique_subjects_names, samples_per_subject, pc_ext='.ply', min_samples=-1, max_samples=-1):
        filtered_pc_paths = []
    ...
    def load_filter_organize_pointclouds_paths(self, dir_path, dataset_name=None, dir_level=2, pc_ext='.ply', min_samples=2, max_samples=-1):
        all_pc_paths, all_pc_subjects, unique_subjects_names, samples_per_subject, indexes_samples = self.get_all_pointclouds_paths_count(dir_path, dataset_name, dir_level=dir_level, pc_ext=pc_ext)
