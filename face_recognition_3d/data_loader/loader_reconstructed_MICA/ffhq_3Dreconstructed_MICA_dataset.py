...
from .tree_ffhq_3Dreconstructed_MICA import TreeFFHQ_3DReconstructedMICA
...
class FFHQ_3D_Reconstructed_MICA_Dataset:
    def __init__(self, other_dataset, root, batch_size = 32, npoints = 2900, min_samples=2, max_samples=-1, split='train', normalize=True, normal_channel=False, modelnet10=False, cache_size=15000, shuffle=None):
        ...
        dataset_name = 'mica_ffhq'

        if not os.path.isfile(paths_file):
            subjects_with_pc_paths, unique_subjects_names, samples_per_subject = TreeFFHQ_3DReconstructedMICA().load_filter_organize_pointclouds_paths(self.root, dataset_name, dir_level, file_ext, min_samples, max_samples)
            # sys.exit(0)
        ...
        if split=='train':
            ...
            for samp_per_subj, uniq_subj_name in zip(samples_per_subject, unique_subjects_names):
                amount_train_samples_subj = max(int(floor(samp_per_subj * 0.8)), 1)
                train_subj_with_paths = []
                for i in range(last_index, len(subjects_with_pc_paths)):
                    # print(f'subjects_with_pc_paths[i][0]: {subjects_with_pc_paths[i][0]}    uniq_subj_name: {uniq_subj_name}')
                    if subjects_with_pc_paths[i][0] == uniq_subj_name:
                        ...
                        if len(train_subj_with_paths) == amount_train_samples_subj:
                            last_index = i
                            break
                assert len(train_subj_with_paths) == amount_train_samples_subj, f'Error, len(train_subj_with_paths) ({len(train_subj_with_paths)}) != amount_train_samples_subj ({amount_train_samples_subj}). They must be equal!'
            ...
            for samp_per_subj, uniq_subj_name in zip(samples_per_subject, unique_subjects_names):
                ...
                # print(f'amount_test_samples_subj: {amount_test_samples_subj}')
                test_subj_with_paths = []
                if amount_test_samples_subj > 0:
                    for i in range(last_index+amount_train_samples_subj, len(subjects_with_pc_paths)):
                        if subjects_with_pc_paths[i][0] == uniq_subj_name:
                            ...
                            if len(test_subj_with_paths) == amount_test_samples_subj:
                                last_index = i+1
                                break
                # print('test_subj_with_paths:', test_subj_with_paths)
                assert len(test_subj_with_paths) == amount_test_samples_subj, f'Error, len(test_subj_with_paths) ({len(test_subj_with_paths)}) != amount_test_samples_subj ({amount_test_samples_subj}). They must be equal!'
                self.datapath += test_subj_with_paths

        self.cat = unique_subjects_names
        if not self.other_dataset is None:
            self.cat = self.other_dataset.cat + self.cat
            self.datapath = self.other_dataset.datapath + self.datapath
