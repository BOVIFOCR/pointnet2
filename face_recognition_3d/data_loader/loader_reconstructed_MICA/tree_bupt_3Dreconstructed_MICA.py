...
# BERNARDO
class TreeBUPT_3DReconstructedMICA:
    '''
    ...
    def load_all_pairs_samples_from_protocol_file(self, protocol_file_path='pairs.txt', dataset_path='', file_ext='.npy'):
        ...
        with open(protocol_file_path, 'r') as fp:
            ...
            # print('all_lines:', all_lines)
            # sys.exit(0)
            total_num_pairs = len(all_lines)
            all_pairs_paths_label = [None] * total_num_pairs
            num_pos_pairs, num_neg_pairs = 0, 0

            global_pair_idx = 1
            for i, line in enumerate(all_lines):
                sample1, sample2 = line.split(';')
                sub_path1 = sample1.replace(src_ext, '')
                sub_path2 = sample2.replace(src_ext, '')
                subj1_folder_path = os.path.join(dataset_path, sub_path1, '*'+file_ext)
                subj2_folder_path = os.path.join(dataset_path, sub_path2, '*'+file_ext)
                path_sample1 = glob(subj1_folder_path)
                path_sample2 = glob(subj2_folder_path)
                assert len(path_sample1) > 0, f'Error, no file found in path_sample1=\'{subj1_folder_path}\''
                assert len(path_sample1) > 0, f'Error, no file found in path_sample1=\'{subj2_folder_path}\''
                ...
                race1 = sample1.split('/')[0]
                subj1 = sample1.split('/')[1]
                race2 = sample2.split('/')[0]
                subj2 = sample2.split('/')[1]

                if race1 == race2 and subj1 == subj2:
                ...
                pair = (pair_label, path_sample1, path_sample2)
                all_pairs_paths_label[i] = pair
                print(f'pair {i+1}/{len(all_lines)}    num_pos_pairs: {num_pos_pairs}    num_neg_pairs: {num_neg_pairs}', end='\r')
