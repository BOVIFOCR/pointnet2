...
from .tree_casia_3Dreconstructed_MICA import TreeCASIA_3DReconstructedMICA
...
class CASIA_3D_Reconstructed_MICA_Dataset:
    def __init__(self, other_dataset, root, batch_size = 32, npoints = 2900, min_samples=2, max_samples=-1, split='train', normalize=True, normal_channel=False, modelnet10=False, cache_size=15000, shuffle=None):
        ...
        dataset_name = 'mica_casia'

        if not os.path.isfile(paths_file):
            subjects_with_pc_paths, unique_subjects_names, samples_per_subject = TreeCASIA_3DReconstructedMICA().load_filter_organize_pointclouds_paths(self.root, dataset_name, dir_level, file_ext, min_samples, max_samples)
            # sys.exit(0)
